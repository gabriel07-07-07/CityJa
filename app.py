...
BASE=os.path.dirname(os.path.abspath(__file__)); DB=os.path.join(BASE,"cityja.db")
...
def init_db():
    ...
    if c.execute("SELECT COUNT(*) FROM users").fetchone()[0]==0:
        uid=c.execute("INSERT INTO users(name,email,password_hash,role) VALUES(?,?,?,?)",
                      ("CityJá Demo","demo@cityja.local",generate_password_hash("cityja123"),"merchant")).lastrowid
        samples=[
        ...
        for n,cat,city,addr,desc,lat,lng in samples:
            c.execute("INSERT INTO businesses(owner_id,name,category,city,address,description,lat,lng) VALUES(?,?,?,?,?,?,?,?)",(uid,n,cat,city,addr,desc,lat,lng))
        bid=c.execute("SELECT id FROM businesses WHERE name='Bar do Zé'").fetchone()[0]
        c.execute("INSERT INTO promotions(business_id,title,description,discount,valid_until) VALUES(?,?,?,?,?)",(bid,"Happy Hour","Chopp + petisco de segunda a quinta","20%","2026-12-31"))
        c.execute("INSERT INTO events(business_id,title,description,event_date) VALUES(?,?,?,?)",(bid,"Música ao vivo","Sexta com banda local","2026-09-04 20:00"))
    c.commit(); c.close()

def auth():
    t=request.headers.get("Authorization","").replace("Bearer ","").strip()
    if not t:return None
    c=db(); u=c.execute("SELECT u.* FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token=?",(t,)).fetchone(); c.close(); return u
