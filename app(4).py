...
import os, re, base64, io, json, hashlib, smtplib, ssl
...
def geocode_business(address, city, state=""):
    """Tenta obter latitude/longitude a partir do endereço usando OpenStreetMap Nominatim."""
    address = (address or "").strip()
    city = (city or "").strip()
    state = (state or "").strip().upper()
    if not address or not city:
        return None, None
    query = f"{address}, {city}, {state}, Brasil" if state else f"{address}, {city}, Brasil"
    try:
        url = "https://nominatim.openstreetmap.org/search?" + urlencode({
            "q": query, "format": "jsonv2", "limit": 1, "countrycodes": "br"
        })
        req = Request(url, headers={"User-Agent": "CityJa/1.0 (cityja app)"})
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS password_resets(id SERIAL PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,channel TEXT NOT NULL,token_hash TEXT NOT NULL,code_hash TEXT,expires_at TIMESTAMPTZ NOT NULL,used BOOLEAN DEFAULT FALSE,created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);
    """)
...
def register():
    d=request.get_json(force=True); name=(d.get("name") or "").strip(); email=(d.get("email") or "").strip().lower(); phone=(d.get("phone") or "").strip(); pw=d.get("password") or ""; role=d.get("role","user")
    ...
    c=db(); cur=c.cursor()
    try:
        cur.execute("INSERT INTO users(name,email,phone,password_hash,role) VALUES(%s,%s,%s,%s,%s) RETURNING id",(name,email,phone,generate_password_hash(pw),role)); uid=cur.fetchone()[0]
        t=token_urlsafe(32); cur.execute("INSERT INTO sessions(token,user_id) VALUES(%s,%s)",(t,uid)); c.commit()
        return jsonify(token=t,user={"id":uid,"name":name,"email":email,"phone":phone,"role":role}),201
...
@app.post("/api/login")
def login():
    d=request.get_json(force=True); email=(d.get("email") or "").strip().lower(); phone=(d.get("phone") or "").strip(); pw=d.get("password") or ""
    c=db(); cur=c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    if phone:
    ...
    u=cur.fetchone()
    if not u or not check_password_hash(u["password_hash"],pw): cur.close(); c.close(); return jsonify(error="E-mail/telefone ou senha incorretos."),401
    t=token_urlsafe(32); cur.execute("INSERT INTO sessions(token,user_id) VALUES(%s,%s)",(t,u["id"])); c.commit(); cur.close(); c.close()
    return jsonify(token=t,user={"id":u["id"],"name":u["name"],"email":u["email"],"phone":u.get("phone"),"role":u["role"]})

@app.get("/api/me")
def me():
    u=auth()
    if not u:return jsonify(error="Não autenticado."),401
    return jsonify(user={"id":u["id"],"name":u["name"],"email":u["email"],"phone":u.get("phone"),"role":u["role"]})
...
def _send_email(to_email, subject, body):
    ...
    if not all([host,user,password,sender]):
        raise RuntimeError("Recuperação por e-mail ainda não está configurada no Render (SMTP_HOST, SMTP_USER, SMTP_PASSWORD e SMTP_FROM).")
...
def _send_sms(to_phone, text):
    ...
    if not all([sid,auth,sender]):
        raise RuntimeError("Recuperação por SMS ainda não está configurada no Render (TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN e TWILIO_FROM_NUMBER).")
    import base64 as _b64
    data=urlencode({"To":to_phone,"From":sender,"Body":text}).encode()
    req=Request(f"https://api.twilio.com/2010-04-01/Accounts/{sid}/Messages.json",data=data,method="POST")
    req.add_header("Authorization","Basic "+_b64.b64encode(f"{sid}:{auth}".encode()).decode())
    req.add_header("Content-Type","application/x-www-form-urlencoded")
    with urlopen(req,timeout=20) as resp: resp.read()

def _create_reset(user_id, channel):
    token=token_urlsafe(32)
    code=f"{__import__('secrets').randbelow(1000000):06d}" if channel=='sms' else None
    c=db(); cur=c.cursor()
    cur.execute("UPDATE password_resets SET used=TRUE WHERE user_id=%s AND used=FALSE",(user_id,))
    cur.execute("INSERT INTO password_resets(user_id,channel,token_hash,code_hash,expires_at) VALUES(%s,%s,%s,%s,NOW()+INTERVAL '15 minutes')",(user_id,channel,_sha(token),_sha(code) if code else None))
    c.commit(); cur.close(); c.close()
    return token,code

@app.post("/api/password/forgot")
def password_forgot():
    d=request.get_json(force=True) or {}; email=(d.get("email") or "").strip().lower(); phone=(d.get("phone") or "").strip()
    c=db(); cur=c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    if phone: cur.execute("SELECT * FROM users WHERE phone=%s",(phone,)); channel='sms'
    else: cur.execute("SELECT * FROM users WHERE email=%s",(email,)); channel='email'
    u=cur.fetchone(); cur.close(); c.close()
    if not u: return jsonify(message="Se os dados estiverem cadastrados, as instruções de recuperação serão enviadas.")
    token,code=_create_reset(u['id'],channel)
    if channel=='email':
        base=os.environ.get('APP_BASE_URL','').rstrip('/')
        if not base: raise RuntimeError("APP_BASE_URL não configurada no Render.")
        link=base+'/?reset='+token
        body=f"Olá, {u['name']}!\n\nRecebemos uma solicitação para redefinir sua senha do CityJá.\n\nAbra este link para criar uma nova senha:\n{link}\n\nO link expira em 15 minutos. Se você não pediu isso, ignore este e-mail."
        _send_email(u['email'],'Redefinição de senha — CityJá',body)
        return jsonify(message="Se o e-mail estiver cadastrado, o link de recuperação foi enviado.",channel='email')
    _send_sms(phone,f"CityJá: seu código para redefinir a senha é {code}. Ele expira em 15 minutos.")
    return jsonify(message="Código enviado por SMS.",channel='sms',reset_token=token)

@app.post("/api/password/reset")
def password_reset():
    d=request.get_json(force=True) or {}; token=d.get('token') or ''; code=(d.get('code') or '').strip(); pw=d.get('password') or ''
    if len(pw)<6:return jsonify(error="A nova senha deve ter pelo menos 6 caracteres."),400
    c=db(); cur=c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT * FROM password_resets WHERE token_hash=%s AND used=FALSE AND expires_at>NOW() ORDER BY created_at DESC LIMIT 1",(_sha(token),)); r=cur.fetchone()
    if not r:cur.close();c.close();return jsonify(error="Link ou código inválido/expirado."),400
    if r['channel']=='sms' and (not code or r['code_hash']!=_sha(code)):
        cur.close();c.close();return jsonify(error="Código SMS inválido."),400
    cur.execute("UPDATE users SET password_hash=%s WHERE id=%s",(generate_password_hash(pw),r['user_id']))
    cur.execute("UPDATE password_resets SET used=TRUE WHERE id=%s",(r['id'],)); c.commit(); cur.close(); c.close()
...
def geocode_api():
    ...
    if not address or not city or not state:
        return jsonify(error="Endereço, cidade e estado são obrigatórios."),400
    lat,lng=geocode_business(address,city,state)
...
def create_business():
    ...
    if not d.get("name") or not d.get("category") or not d.get("city") or not d.get("state"):return jsonify(error="Nome, categoria, cidade e estado são obrigatórios."),400
    lat=d.get("lat")
    lng=d.get("lng")
    if lat in (None, "") or lng in (None, ""):
        lat, lng = geocode_business(d.get("address", ""), d.get("city", ""), d.get("state", ""))
    c=db(); cur=c.cursor(); cur.execute("INSERT INTO businesses(owner_id,name,category,city,state,address,description,lat,lng,phone,website,photo,whatsapp,hours,price_range) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",(u["id"],d["name"],d["category"],d["city"],d["state"].upper(),d.get("address",""),d.get("description",""),lat,lng,d.get("phone",""),d.get("website",""),d.get("photo",""),d.get("whatsapp",""),d.get("hours",""),d.get("price_range",""))); bid=cur.fetchone()[0]; c.commit(); cur.close(); c.close(); return jsonify(id=bid,lat=lat,lng=lng),201
...
def update_business(bid):
    ...
    allowed=("name","category","city","state","address","description","lat","lng","phone","website","photo","whatsapp","hours","price_range")
    fields=[]; vals=[]
    for k in allowed:
        if k in d:
            if k in ("lat", "lng") and d[k] in (None, ""):
                continue
            if k=="photo" and d[k]:
    ...
    # Se o comerciante alterou o endereço, ou ainda não possui coordenadas,
    # tenta localizar automaticamente a loja para o mapa/Perto de mim.
    current = None
    cur.execute("SELECT city,state,address,lat,lng FROM businesses WHERE id=%s", (bid,))
    current = cur.fetchone()
    target_city = d.get("city", current["city"] if current else "")
    target_address = d.get("address", current["address"] if current else "")
    target_state = d.get("state", current.get("state", "") if current else "")
    target_lat = d.get("lat", current["lat"] if current else None)
    target_lng = d.get("lng", current["lng"] if current else None)
    location_changed = bool(current) and any([
        "city" in d and str(d.get("city", "")).strip() != str(current.get("city") or "").strip(),
        "state" in d and str(d.get("state", "")).strip().upper() != str(current.get("state") or "").strip().upper(),
        "address" in d and str(d.get("address", "")).strip() != str(current.get("address") or "").strip(),
    ])
    if (location_changed or target_lat in (None, "") or target_lng in (None, "")) and target_address and target_city:
        geo_lat, geo_lng = geocode_business(target_address, target_city, target_state)
        if geo_lat is not None and geo_lng is not None:
            fields.extend(["lat=%s", "lng=%s"])
            vals.extend([geo_lat, geo_lng])
