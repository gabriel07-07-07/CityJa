...
def geocode_business(address, city):
...
def update_business(bid):
    ...
    allowed=("name","category","city","address","description","lat","lng","phone","website","photo","whatsapp","hours","price_range")
    fields=[]; vals=[]
    for k in allowed:
        if k in d:
    ...
    current = None
    cur.execute("SELECT city,address,lat,lng FROM businesses WHERE id=%s", (bid,))
    ...
    if (target_lat in (None, "") or target_lng in (None, "")) and target_address and target_city:
        geo_lat, geo_lng = geocode_business(target_address, target_city)
